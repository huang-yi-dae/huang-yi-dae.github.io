...
1. **Merge constraints into objective function** using _Lagrange multipliers_
2. **Construct another optimization problem** (the _dual problem_) by exchanging the order of minimization and maximization.
...
A natural idea is :
_Since the constrainted problem is hard, can we transform it into an unconstrainted one _

One simple approach is to _penalize constraint violations_.
...
Instead, it is _free to change depending on the candiate solution_ w.
This means the penalty can _adapt to the degree of constraint violation_.
...
This reasonging leads naturally to the _Lagrangian function_

$$
L(w,\beta) = f(w) + \beta h(w)
$$

where $ \beta $ is called a _Lagragian multipler_.
...
Previously, we introduced the _Lagrangian_
...
- 2. The the multiplier $ \beta $ choose a value that _maximizes the Lagranian_.
     Mathematically,
     $$
     \max_{\beta} L(w, \beta)
     $$
...
However, most machine learning problems involve _inequality constraints_.
...
The multiplier $ \alpha $ and $ \beta $ _how strong constraint violations are penalized_.
...
_How are these penalty strengths chosen?_
Before, we have discuss and come to a conclusion that: manually choose and predetermined it is hard and
no flexible enough. We mentioned that we will _let the penalty strengths be chosen automatically by optimization itself_.In next section, we will give out how this happens in detailed.
...
Insteading of fixing the multipliers beforehand, we allow them to be chosen after _a candiate solution_ w _is proposed_
More precisely, for a give w, we compute:

$$

\max\_{\alpha, \beta:\alpha_i \ge 0} L(w, \alpha, \beta)


$$

This means the multipliers choose values that _maxmize the Lagrangian_

Intuitively:

- $ w $ tries to minimize the objective
- multipliers try to _increase the objective whenever constraints are violated_
...
\max\_{\alpha,\beta} L(w,\alpha,\beta) = f(w)
...
\max\_{\alpha,\beta} L(w,\alpha,\beta) = \infty
...
\max\_{\alpha,\beta:\alpha_i \geq 0} L(w,\alpha,\beta)
...
\min*{w} \max*{\alpha,\beta:\alpha_i \geq 0} L(w,\alpha,\beta)


$$

This formulation reveals an intresting interpretation.
The optimization becomes a _two-player game_:
...
- the multipliers try to maxmize the penalty for violations
  ...
  the starting point for _Lagrangian duality_

In next section, we will see that how exchanging the order of optimization leads to
a new optimization problem called the _dual problem_.
...
\min*w \max*{\alpha,\beta : \alpha_i \ge 0} L(w, \alpha, \beta)
...
_What happens if we reverse the order of optimization_
Insteading of sloving:

$$

\min*w \max*{\alpha,\beta} L(w, \alpha, \beta)
...
\max*w \min*{\alpha,\beta} L(w, \alpha, \beta)
...
This function is called the _dual function_
Interpretation:
...
- given that rule, we compute the best possible objective value
  ...
  _Given this penalty rule, what's the smallest value the objective can achieve_

## The Dual Function Gives a Lower Bound

An important observation is that the function always provide a _lower bound_ on the optimal
...
Now recall that the dual function takes the _minimum over all_ w:
...
Since all these bounds are valid the natural strategy is to choose the _largest possible one_
Therefore, we solve

$$

\max\_{\alpha, \beta: \alpha_i \ge 0} \theta_D(\alpha,\beta)


$$

This optimization problem is called the _Lagrange dual problem_

Interpretation:
The dual problem searches for the penalty coefficients that produce the _best possible lower bound_ on the optimal objective value.
Let

$$

d^\* = \max\_{\alpha, \beta: \alpha_i \ge 0} \theta_D (\alpha, \beta)
$$
...
This property is known as _weak duality_
...
_How tight can this bound be?_
...
This difference is called the _duality gap_.
If such a gap exists, solving the dual problem could only gives an _approximation_ of the true optimal value. So, can we rub off the gap? The answer is: Absolutely!
...
This property is called _strong duality_.
When strong duality holds, solving the dual problem is _equivalent_ to solving the primal problem.
So, when does _strong duality_ hold?
...
- the primal problem is _convex_
- the problem satisfies _Slater's condition_
...
_If the primal and dual optima coincide, how do we actually find the optimal solution?_
In other words, how can we recognize a pair of primal and dual variables that achieve this optimum.

The answer is given by a set of conditions known as _Karush-Kuhn-Tucker(KKT) conditions_.
...
a pair ($ w^_, \alpha^_, \beta^* $ is optimal if and only if it satisfies the *KKT conditions*.
...
Among the KKT conditions, _complementary slackness_ is the most conceptually interesting:
...
This ties beautifully back to earlier explanation of _adaptive penalties_.

# 5. Example: Suppor Vector Machine

## How KKT is Used to Solve Problems

Next, we are going to explain that KKT conditions are _not just theory_,
...
Setting the derivatives to zero give the _stationarity conditions_:
...
Substituting these expressings back into the Lagrangian yields the _dual problem_
...
Thus the optimal weight vector is expressed as a _linear combination of the training points_.
...
Only the _support vectors_, the points lying exactly on the margin, have nonzero multipliers.
...
we use the fact that _for any support vector the margin constraint is active_:
...
_the decision boundary depends only on the support vectors_, rather than the entire dataset.
